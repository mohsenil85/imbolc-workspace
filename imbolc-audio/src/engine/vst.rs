...
impl AudioEngine {
    ...
    pub(crate) fn query_vst_param_count_node(&self, node_id: i32) -> Result<(), String> {
        let backend = self.backend.as_ref().ok_or("Not connected")?;
        backend
            .send_unit_cmd(node_id, VST_UGEN_INDEX, "/param_count", vec![])
            .map_err(|e| e.to_string())?;
        Ok(())
    }
    ...
    pub(crate) fn query_vst_params_range(
        &self,
        node_id: i32,
        start: u32,
        count: u32,
    ...
    pub(crate) fn set_vst_param_node(
        &self,
        node_id: i32,
        param_index: u32,
        value: f32,
    ...
    pub(crate) fn save_vst_state_node(
        &self,
        node_id: i32,
        path: &std::path::Path,
    ) -> Result<(), String> {
    ...
    pub(crate) fn load_vst_state_node(
        &self,
        node_id: i32,
        path: &std::path::Path,
    ) -> Result<(), String> {
