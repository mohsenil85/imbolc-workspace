...
fn roundtrip_key() {
    ...
    for &k in &all {
        match k { C | Cs | D | Ds | E | F | Fs | G | Gs | A | As | B => {} }
...
fn roundtrip_scale() {
    ...
    let all = [Major, Minor, Dorian, Phrygian, Lydian, Mixolydian, Aeolian, Locrian, Pentatonic, Blues, Chromatic];
    for &s in &all {
        match s { Major | Minor | Dorian | Phrygian | Lydian | Mixolydian | Aeolian | Locrian | Pentatonic | Blues | Chromatic => {} }
...
fn roundtrip_source_type() {
    ...
    let all = [
        Saw, Sin, Sqr, Tri, Noise, Pulse, SuperSaw, Sync, Ring, FBSin,
        FM, PhaseMod, FMBell, FMBrass, Pluck, Formant,
        Bowed, Blown, Membrane, Marimba, Vibes, Kalimba, SteelDrum, TubularBell, Glockenspiel,
        Guitar, BassGuitar, Harp, Koto,
        Kick, Snare, HihatClosed, HihatOpen, Clap, Cowbell, Rim, Tom, Clave, Conga,
...
fn roundtrip_effect_type() {
    ...
    let all = [
        Delay, Reverb, Gate, TapeComp, SidechainComp,
        Chorus, Flanger, Phaser, Tremolo,
        Distortion, Bitcrusher, Wavefolder, Saturator, TiltEq,
        StereoWidener, FreqShifter, Limiter, PitchShifter,
        Vinyl, Cabinet, GranularDelay, GranularFreeze, ConvolutionReverb,
        Vocoder, RingMod, Autopan, Resonator,
...
fn roundtrip_filter_type() {
    ...
    for &f in &all {
        match f { Lpf | Hpf | Bpf | Notch | Comb | Allpass | Vowel | ResDrive => {} }
...
fn roundtrip_eq_band_type() {
    ...
    for &b in &all {
        match b { LowShelf | Peaking | HighShelf => {} }
...
fn roundtrip_lfo_shape() {
    ...
    for &s in &all {
        match s { Sine | Square | Saw | Triangle => {} }
        let encoded = format!("{:?}", s);
...
fn roundtrip_parameter_target() {
    ...
    let all = [
        ...
        Pitch, PulseWidth, Detune, FmIndex, WavetablePosition,
        FormantFreq, SyncRatio, Pressure, Embouchure,
        GrainSize, GrainDensity, FbFeedback, RingModDepth,
        ChaosParam, AdditiveRolloff, MembraneTension,
        SampleRate, SampleAmp, StretchRatio, PitchShift,
        DelayTime, DelayFeedback, ReverbMix, GateRate,
    ...
    for pt in &all {
        match pt {
            ...
            LfoRate | LfoDepth | Swing |
            HumanizeVelocity | HumanizeTiming | TimingOffset | TimeSignature |
            SendLevel(_) | EffectParam(_, _) | EffectBypass(_) |
            EqBandFreq(_) | EqBandGain(_) | EqBandQ(_) | VstParam(_) => {}
        }
        ...
        assert_eq!(&decoded, pt, "ParameterTarget roundtrip failed for {:?}", pt);
    }
}
...
fn roundtrip_curve_type() {
    ...
    for &ct in &all {
        match ct { Linear | Exponential | Step | SCurve => {} }
...
fn roundtrip_arp_rate() {
    ...
    for &r in &all {
        match r { Quarter | Eighth | Sixteenth | ThirtySecond => {} }
...
fn roundtrip_chord_shape() {
    ...
    let all = [Major, Minor, Seventh, MinorSeventh, Sus2, Sus4, PowerChord, Octave];
    for &cs in &all {
        match cs { Major | Minor | Seventh | MinorSeventh | Sus2 | Sus4 | PowerChord | Octave => {} }
...
fn roundtrip_swing_grid() {
    ...
    for &sg in &all {
        match sg { Eighths | Sixteenths | Both => {} }
...
fn roundtrip_step_resolution() {
    ...
    let all = [Quarter, Eighth, Sixteenth, ThirtySecond];
    for &sr in &all {
        match sr { Quarter | Eighth | Sixteenth | ThirtySecond => {} }
